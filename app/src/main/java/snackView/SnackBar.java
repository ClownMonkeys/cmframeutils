...
public class SnackBar  {
    ...
    private snackView.SnackContainer mSnackContainer;

    private View mParentView;

    private OnMessageClickListener mClickListener;

    public snackView.OnVisibilityChangeListener mVisibilityChangeListener;
    ...
    public SnackBar(Activity activity) {
        ...
        View v = activity.getLayoutInflater().inflate(R.layout.snack_layout, container, false);
        init(container, v);
    }

    public SnackBar(Context context, View v) {
        LayoutInflater inflater = (LayoutInflater) context.getSystemService(Context.LAYOUT_INFLATER_SERVICE);
        inflater.inflate(R.layout.snack_container_layout, ((ViewGroup) v));
        View snackLayout = inflater.inflate(R.layout.snack_layout, ((ViewGroup) v), false);
        init((ViewGroup) v, snackLayout);
    }

    private void init(ViewGroup container, View v) {
        mSnackContainer = (snackView.SnackContainer) container.findViewById(R.id.snackContainer);
        if (mSnackContainer == null) {
            mSnackContainer = new snackView.SnackContainer(container);
        }
    ...
    public static class Builder {
         ...
         * @return this builder
         */
        public Builder withVisibilityChangeListener(snackView.OnVisibilityChangeListener visibilityChangeListener) {
        ...
        public SnackBar show() {
            snackView.Snack message = new snackView.Snack(mMessage,
                    (mActionMessage != null ? mActionMessage.toUpperCase() : null),
                    mActionIcon,
                    mToken,
                    mDuration,
                    mTextColor != null ? mTextColor : getActionTextColor(Style.DEFAULT),
                    mBackgroundColor != null ? mBackgroundColor : mContext.getResources().getColorStateList(R.color.snack_bkgnd),
                    mHeight != 0 ? mHeight : 0,
                    mTypeFace);
        ...
        private ColorStateList getActionTextColor(Style style) {
            switch (style) {
                case ALERT:
                    return mContext.getResources().getColorStateList(R.color.snack_text_color_red);
                case INFO:
                    return mContext.getResources().getColorStateList(R.color.snack_text_color_yellow);
                case CONFIRM:
                    return mContext.getResources().getColorStateList(R.color.snack_text_color_green);
                case DEFAULT:
                    return mContext.getResources().getColorStateList(R.color.snack_text_color_default);
                default:
                    return mContext.getResources().getColorStateList(R.color.snack_text_color_default);
            }
        }
    ...
    private void showMessage(snackView.Snack message) {
    ...
    private SnackBar setOnVisibilityChangeListener(snackView.OnVisibilityChangeListener listener) {
